...
from cs336_basics import modules, utils
...
def run_linear(
    ...
    """
    model = modules.MyLinear(d_in, d_out)
    model.load_state_dict(state_dict={'W': weights.T})
...
def run_embedding(
    ...
    model = modules.MyEmbedding(vocab_size, d_model)
    model.load_state_dict(state_dict={'W': weights})
...
def run_transformer_block(
    ...
    """
    model = modules.MyTransformerBlock(d_model=d_model, num_heads=num_heads, d_ff=d_ff, max_seq_len=max_seq_len, theta=theta)
    model.attn.q_proj.data.copy_(weights["attn.q_proj.weight"])
    model.attn.k_proj.data.copy_(weights["attn.k_proj.weight"])
    model.attn.v_proj.data.copy_(weights["attn.v_proj.weight"])
    model.attn.o_proj.data.copy_(weights["attn.output_proj.weight"])
    model.attnorm.W.data.copy_(weights["ln1.weight"])
    model.ffnnorm.W.data.copy_(weights["ln2.weight"])
...
def run_rmsnorm(
    ...
    model = modules.MyRMSNorm(d_model, eps)
    model.load_state_dict(state_dict={'W': weights})
...
def run_get_batch(
    ...
    """
    return utils.data_loading(dataset, batch_size, context_length, device)
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    return modules.MySoftmax(in_features, dim)
...
def run_cross_entropy(
    ...
    """
    return modules.MyCrossEntropy(inputs, targets)
...
def run_get_lr_cosine_schedule(
    ...
    """
    return utils.lr_scheduler(it, max_learning_rate, min_learning_rate, warmup_iters, cosine_cycle_iters)
